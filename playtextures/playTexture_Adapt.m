...
for i = 1:Nadaptframes
    %generate motion - this can continuously go in one direction, we'll
    %handle the reversal with the orientation
    xoffset2 = mod((i-1)*shiftperframe2+P.phase2/360*pixpercycle2,pixpercycle2);
    stimSrc2=[xoffset2 0 xoffset2 + stimsizeN2 stimsizeN2];
    
    %stimulus reversals
    if mod(i-1,P.cycPerDir*P.t_period2)==0
        oribit=~oribit;
    end
    ori=P.ori2+oribit*180;
    
    %for individual grating or plaid, plot grating1
    Screen('BlendFunction', screenPTR, GL_SRC_ALPHA, GL_ONE);
    Screen('DrawTexture', screenPTR, Gtxtr(2), stimSrc2, stimDst2,ori,[],ctr2);
