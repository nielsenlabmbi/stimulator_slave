function playTexture_PerGratingFG
...
%adds function generator output
...
Nstartfg = ceil(P.startfg*screenRes.hz)+1; 
if P.trialPhaseStop==0 %stop fg is relative to stimulus period
    Nstopfg = Nstimframes-ceil(P.stopfg*screenRes.hz);
else %stop fg is relative to post stim period
    Nstopfg = Npostframes-ceil(P.stopfg*screenRes.hz)-1;
end
...
for i = 1:Npreframes
    ...
    if loopTrial ~= -1 && ~isempty(daq) && i==1
