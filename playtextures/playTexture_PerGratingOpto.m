function playTexture_PerGratingOpto
...
Noptoframes = ceil(P.optodelay*screenRes.hz);
Npreframes = ceil(P.predelay*screenRes.hz)-Noptoframes;
Npostframes = ceil(P.postdelay*screenRes.hz)-Noptoframes;
...
%now start optogenetic stimulation
for i = 1:Noptoframes
    if loopTrial ~= -1 && ~isempty(daq) && P.opto_bit==1 && i==1
        digWord = 5;  %Make 1st and 3rd bit high    
        DaqDOut(daq, 0, digWord);
    end
    Screen('DrawTexture', screenPTR, Stxtr(2),syncSrc,syncDst);
    Screen(screenPTR, 'Flip');
...
for i = 1:Nstimframes
    ...
    %generate event
    if ~isempty(daq)
        if P.opto_bit==1   %optostim stays high
           if i==1 && loopTrial ~= -1
                digWord=7;
                DaqDOut(daq, 0, digWord);
            end
...
%%%Play postdelay - with stimulation %%%%
Screen(screenPTR, 'FillRect', P.background) 
for i = 1:Noptoframes
    Screen('DrawTexture', screenPTR, Stxtr(2),syncSrc,syncDst);
    Screen(screenPTR, 'Flip');
    if i==1 && loopTrial ~= -1 && ~isempty(daq)
        if P.opto_bit==1
            digWord = 5;  %toggle 2nd bit to signal stim off
        else
